...
- Curated `datasets/DATA_*.csv` files can be generated by exporting rows from `bot_logs`, `extraction_results`, `extraction_comparison`, `logging_comparison`, `llm_evaluation_results`, `llm_evaluation_results_exploratory`, `llm_eval_comparison`, `llm_eval_comparison_exploratory`, `v_macro_f1_scores`, and `v_macro_f1_scores_exploratory`.
- For reproducible exports, use `npm run export:datasets` (script: `scripts/export-datasets.mjs`) instead of manual UI downloads.
