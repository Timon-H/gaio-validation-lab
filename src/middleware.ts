...
const AI_BOTS = [
  {
    name: "ChatGPT",
    regex: /GPTBot|OAI-SearchBot|ChatGPT-User|ChatGPT Agent/i,
  },
  {
    name: "Claude",
    regex: /ClaudeBot|Claude-Web|Claude-User|Claude-SearchBot|anthropic-ai/i,
  },
  {
    name: "Gemini",
    regex:
      /Google-Extended|Gemini-Deep-Research|Google-NotebookLM|NotebookLM|GoogleAgent-Mariner/i,
  },
  ...
  { name: "Applebot", regex: /Applebot/i },
  {
    name: "Meta",
    regex:
      /meta-externalagent|Meta-ExternalAgent|meta-externalfetcher|meta-webindexer/i,
  },
...
export const onRequest: MiddlewareHandler = async (context, next) => {
  ...
  const matchedVariant = VARIANTS.find((variant) =>
    url.pathname.startsWith(variant.path),
  );
  if (!matchedVariant) {
    return next();
  }
  ...
  if (isAiBot && hasSupabaseConfig) {
    ...
    const result = await supabaseInsert("bot_logs", logData, {
